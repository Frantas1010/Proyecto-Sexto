...
exports.consulta = (req, res, message) => {
    const imagePath = path.join(__dirname, "..", "public", "imagenes", "hola.jpeg");

    openai.createCompletion({
        model: "text-davinci-003",
        prompt: `quiero que me generes un cuento en base a mi siguiente prompt que contenga:
        "--titulo--"{genera titulo de cuento}"--fintitulo--"
        "--contenido--"{genera cuento}"--fincontenido--"
        "--genero--"{lista de generos literarios}"--fingenero--"` + message,
        temperature: 0.9,
        max_tokens: 3900,
        top_p: 1,
        frequency_penalty: 0,
        ...
        const contenido = texto.slice(texto.indexOf('--contenido--'.normalize('NFD').replace(/[\u0300-\u036f]/g, '')) + '--contenido--'.length, texto.indexOf('--fincontenido--'.normalize('NFD').replace(/[\u0300-\u036f]/g, '')));
        const genero = texto.slice(texto.indexOf('--genero') + '--genero'.length + 7, texto.indexOf('{--fingenero'));
...
function TextoPDF(texto, imagePath) {
    ...
    if (fs.existsSync(imagePath)) {
        ...
        console.error("La imagen no se encuentra en la ruta especificada:", imagePath);
        return;
    }
}
